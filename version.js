...
(function() {
    ...
    // 创建并插入执行脚本按钮到每个目标 button 前
    function addScriptButtons() {
        // 获取所有符合条件的按钮，这些将是所谓的 button1
        ...
        allButtons.forEach(function(targetButton) {
            // 确保每个目标按钮只有一个相邻的执行脚本按钮
            if (targetButton.previousElementSibling && targetButton.previousElementSibling.textContent === '执行脚本') {
                return; // 跳过已经有执行脚本按钮的按钮
            }
    ...
    function startScript(button1) {
        ...
        // 询问用户输入的质量值
        var targetQualityValue = prompt('请输入目标质量值:', '17');
        targetQualityValue = parseInt(targetQualityValue, 10);

        // 检查质量值输入是否有效，如果无效则终止脚本
        if (isNaN(targetQualityValue) || targetQualityValue <= 0) {
            alert('输入无效，脚本已终止');
            console.error('输入无效，脚本终止');
            return;
        }

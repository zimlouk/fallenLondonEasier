...
#fl-item-tracker-sidebar .fl-tracker-arrow {
    width: 100%;
    height: 38px;
    background: linear-gradient(to right, #e3c98c 85%, #cab87b 100%);
    color: #6a5321;
    font-size: 24px;
    border: 1px solid #b9a365;
    border-left: none;
    border-radius: 0;              /* 变成尖角 */
    display: flex;
    align-items: center;
    justify-content: flex-end;
    cursor: pointer;
    box-shadow: 0 1px 5px rgba(120, 110, 80, 0.12);
    transition: background 0.16s, color 0.16s;
    margin-bottom: 8px;            /* 与item间距一致 */
    padding-right: 4px;
    box-sizing: border-box;
}
...
#fl-item-tracker-sidebar .fl-tracker-item-container {
    ...
    flex-direction: column;
    align-items: center;
    box-shadow: 0 1px 3px rgba(170,150,100,0.08);
}
...
#fl-item-tracker-sidebar .tracker-quantity {
  ...
  function setItemTarget(itemId, targetValue) {
    ...
    if (
      ...
      if (!isNaN(targetNum) && targetNum >= 0) {
        ...
        console.log(
          `FL Tracker: Setting target for item ${itemId} to ${targetNum}`
        );
      } else {
        console.warn(
    ...
    document.addEventListener(
      ...
      (event) => {
        ...
        if (!clickedInsideSidebar && !clickedInsideVisibleTooltip) {
          console.log(
            "FL Tracker: Click outside active container/tooltip, hiding tooltip."
          );
          visibleTooltip.classList.remove(VISIBLE_CLASS);
          showStatusMessage(visibleTooltip, "", null); // Clear status
        }
  ...
  function updateTrackedItemDisplay(itemId, newQuantity) {
    ...
    if (quantitySpan) {
      ...
      if (quantitySpan.textContent !== formattedQuantity) {
        console.log(
          `FL Tracker: Updating display for ${itemId} to "${formattedQuantity}"`
        );
  ...
  function displayTrackedItems() {
    ...
    if (!displayBar) {
        ...
        arrow.addEventListener("click", function(e) {
            e.stopPropagation();
            displayBar.classList.toggle("fl-tracker-collapsed");
    ...
    if (trackedIds.length === 0) {
        ...
        placeholder.className = 'fl-tracker-placeholder';
        ...
        fetchPromise
          .then((response) => {
            if (response.ok) {
              response
                .clone()
                .json() // Clone before reading body
    ...
    originalXHR.prototype.send = function () {
      ...
      xhr.onreadystatechange = function () {
        if (
          ...
          processed = true;
          console.log(
            `FL Tracker: Intercepting XHR: ${xhr._trackedUrl} (Status: ${xhr.status})`
          );
    ...
    GM_xmlhttpRequest({
      ...
      onload: function (response) {
        ...
        if (response.status >= 200 && response.status < 300) {
          console.log(
            "FL Tracker: API use successful. Page should update via intercept/reload."
          );
        ...
        if (missingIds.length > 0) {
          ...
          missingIds.forEach((missingId) => {
            ...
            if (
              ...
              console.log(
                `FL Tracker: Setting quantity to 0 for missing item ID ${missingId}.`
              );
  ...
  function enhancePossessionsPage() {
    ...
    if (itemIcons.length > 0) {
      console.log(
        `FL Tracker: Found ${itemIcons.length} new item icons to enhance.`
      );
    }
    ...
    const observerCallback = function (mutationsList, observer) {
      ...
      for (const mutation of mutationsList) {
        if (
          ...
          const affectedIcons =
            ...
            ); // Don't need querySelector for removed
          if (affectedIcons) {
            needsEnhance = true;
            break;
          }
      ...
      if (needsEnhance) {
        ...
        enhanceDebounceTimer = setTimeout(() => {
          ...
          console.log(
            "FL Tracker: Temporarily disconnecting observer for enhancement."
          );
          observer.disconnect(); // Use the passed observer instance
          try {
            console.log(
              "FL Tracker: Re-enhancing possessions page after mutation."
            );
  ...
  function handlePathChange() {
    ...
    if (possessionsObserver) {
      console.log(
        "FL Tracker: Disconnecting Possessions observer due to path change."
      );
      possessionsObserver.disconnect();
      possessionsObserver = null;
    }
